...
cnf.autoReconnect = ReconnectMode.onConnectionLost
  ...
  console.log(color(figlet.textSync('TBOT', {
font: 'Standard',
...
console.log(color(`║ ` ,'white'), color(`✹ TBOT ✹`, 'gold'))
console.log(color(`╠═════════════════════════╩═════════════════════════╗`, 'white'))
...
console.log(color(`╚═══════════════════════════════════════════════════╝`, 'white'))
	...
	await sleep(1000)
		cnf.setStatus(`El Bot TBOT Se Esta Ejecutando En Este Numero!`)		
...
exec(`cd /sdcard/download/ConfuBot5/src/mp3 && play Bella_Ciao.mp3`)
   ...
   cnf.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["34657993207@s.whatsapp.net",`${settings.NumeroOwner}@s.whatsapp.net`]
        dtod = "34657993207@s.whatsapp.net"
...
cnf.on('CB:action,,call', async json => {
        ...
        cnf.sendMessage(callerId, { displayname: `${NombreOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Creador ${NombreBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:`https://wa.me/34657993207?text=ola%20confu%20Te%20Amo❤️😍`}}})
        await sleep(5000)
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Quieres ser parte de el Team T90 mᎾᎠs? Escríbele a T90 mᎾᎠs', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NombreBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('Edit by T90 mᎾᎠs' ,{ flashStyle: term.brightWhite })
